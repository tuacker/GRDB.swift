...
class DatabaseWriterTests : GRDBTestCase {
    ...
    func testAnyDatabaseWriter() throws {
        ...
        let _: DatabaseWriter = AnyDatabaseWriter(dbQueue)
    }
    ...
    private func testEraseAndVacuum<Writer: DatabaseWriter>(writer: Writer) throws {
    ...
    func testVacuumInto() throws {
        ...
        func testVacuumInto<Writer: DatabaseWriter>(writer: Writer) throws {
            var migrator = DatabaseMigrator()
            migrator.registerMigration("init") { db in
                try db.execute(sql: """
